    ...
    for (uint8_t i = 0; i < Hoymiles.getNumInverters(); i++) {
        ...
        for (auto& t : inv->Statistics()->getChannelTypes()) {
            ...
            for (auto& c : inv->Statistics()->getChannelsByType(t)) {
                ...
                addField(chanTypeObj, inv, t, c, FLD_PAC, "", brief);
                addField(chanTypeObj, inv, t, c, FLD_UAC, "", brief);
                addField(chanTypeObj, inv, t, c, FLD_IAC, "", brief);
                if (brief)
                    continue;
                if (t == TYPE_AC) {
...
void WebApiWsLiveClass::addField(JsonObject& root, std::shared_ptr<InverterAbstract> inv, const ChannelType_t type, const ChannelNum_t channel, const FieldId_t fieldId, String topic, bool brief)
{
